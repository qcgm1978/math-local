JS.ENV.LineSpec = JS.Test.describe("Line", function () {
  with (this) {
    before(function () {
      with (this) {
    ...
    test(` there exist a series of shorthand functions for creating objects in Sylvester`, function () {
      with (this) {
        assert($L && $V && $M && $P)
        assert(Sylvester.precision.eql())
      }
    })
    test("dup", function () {
      with (this) {
    ...
    test("equality with antiparallel lines", function () {
      with (this) {
    ...
    test("intersectionWith", function () {
      with (this) {
